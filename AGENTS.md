...
All agents should read this file before executing any edits.
...
## Multi-version port status

| Branch | MC version | Source changes | Notes |
|---|---|---|---|
| `main` | 1.20.1 | — (base) | Stable |
| `port/1.20` | 1.20 | None required | YACL 3.2.2 from `libs/` (no Maven build); ModMenu 7.0.1; loader 0.18.5; fabric-api 0.83.0+1.20; `fabric.mod.json` minecraft constraint `~1.20` |
| `port/1.21.1` | 1.21.1 | Input struct, networking v2, Java 21, HudRenderCallback sig | Automobility 0.5.0.h runtime; ✅ tested working |

**Porting workflow established (2026-03-31):**
- Jars in `libs/` are git-tracked (not ignored) — branch switch automatically swaps them
- `fabric.mod.json` minecraft constraint must match the target version on each branch
- YACL may not have a Maven build for every MC version — fall back to placing the jar in `libs/`
- `run/mods/` must be emptied when switching MC versions — all required mods load via Gradle
- **Loom does NOT extract Jar-in-Jar (JiJ) deps** — any mod that bundles dependencies via JiJ will silently fail in dev. Extract the nested jars from `META-INF/jars/` and add them as explicit `modLocalRuntime` entries in `build.gradle`. This is why 1.20.1 needed jsonem in `libs/` and why 1.21.1 needs obj-0.4.0 and jsonem explicitly declared.

### port/1.21.1 source changes (2026-03-31)

**AutomobileEntityMixin.java — Input struct refactor**
- Automobility 0.5.0 moved `accelerating`, `braking`, `holdingDrift`, `steering` off the entity into `AutomobileEntity.Input input`
- Removed `@Shadow boolean steeringLeft`, `@Shadow boolean steeringRight`
- Added `@Shadow public AutomobileEntity.Input input`
- `steeringLeft || steeringRight` → `input.steering != 0`
- `(steeringLeft && driftDir < 0) || (steeringRight && driftDir > 0)` → `(input.steering < 0 && driftDir < 0) || (input.steering > 0 && driftDir > 0)`

**Injection point audit — all clear for 1.21.1 (verified against 0.5.0.b source)**
- `AUtils.zero` ordinal 1 (coast), `Math.max` ordinal 3 (braking), `AUtils.shift` ordinal 1 (postMovementTick), `0.42f` in `steeringTick`, `hSpeed > 0.5` gate, `driftingTick` signature — all unchanged
- `AutomobileHud` still enum, `renderSpeedometer` still `private static`

**Automobility jar situation**
- 0.5.0.b: source available (zip), but missing `HornSoundDefinition` at runtime — crashes on load
- 0.5.0.h: complete runtime jar; compile and run both use this
- Both jars kept in `libs/` for reference; `build.gradle` references 0.5.0.h explicitly by filename
- JiJ deps extracted from 0.5.0.h and declared explicitly: `obj-0.4.0.jar`, `jsonem-0.3+1.21.jar`

**Networking rewrite (Fabric API v2)**
- `KeyStatePacket` → `record implements CustomPayload` with `PacketCodec.tuple`; `Identifier.of(...)` replaces `new Identifier(...)`
- `Momentum.java`: `PayloadTypeRegistry.playC2S().register(ID, CODEC)` + typed `ServerPlayNetworking.registerGlobalReceiver(ID, (payload, context) -> ...)`
- `MomentumClient.java`: `ClientPlayNetworking.send(new KeyStatePacket(...))` — `PacketByteBufs` removed

**HudRenderCallback signature**
- `(drawContext, tickDelta)` → `(drawContext, tickCounter)` where `tickCounter` is `RenderTickCounter`
- `tickDelta` extracted via `tickCounter.getTickDelta(true)`

**Sound files**
- `new Identifier("automobility", "entity.automobile.skid")` → `Identifier.of(...)` in all 4 skid sound classes

**build.gradle extras**
- Loom bumped `1.7-SNAPSHOT` → `1.13-SNAPSHOT` (Automobility 0.5.0 was built with Loom 1.13)
- Gradle wrapper bumped 8.8 → 8.14 (Loom 1.13 requires Gradle ≥ 8.14)

*— Agent Sonnet 4.6 (2026-03-31)*
